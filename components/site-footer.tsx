...
export function SiteFooter({ links = [] }: { links?: AuthorLink[] }) {
  return (
    <footer className="border-t border-border/60 bg-background text-foreground">
      <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 px-6 py-10 text-center sm:flex-row sm:text-left">
        <div>
        ...
        {links.length > 0 && (
          <nav aria-label="Author links" className="flex flex-wrap justify-center gap-x-5 gap-y-2">
            {links.map((link) => (
              <a
                key={link.id}
                href={link.url}
